...
public class PoolManager :Singleton<PoolManager>
{
    ...
    private readonly List<PoolRegister>                  poolRegisters  = new List<PoolRegister>();

    private int m_Length;

    public PoolManager RegisterPoolExtension(PoolRegister register)
    {
        if (poolRegisters.Contains(register))
        {
            return this;
        }

        poolRegisters.Add(register);
        m_Length++;
    ...
    {
        if (!poolRegisters.Contains(remover))
        {
            return this;
        }

        poolRegisters.Remove(remover);
        m_Length--;
    ...
    {
        for (int i = 0; i < poolRegisters.Count; i++)
        {
            var poolReturn = poolRegisters[i].InstancePool(poolId);
...
public static class PoolExtension
{
    ...
    {
        return PoolManager.Instance.GetPools(poolId, IsEnable);
    }
    ...
    public static PoolManager RemovePoolExtension(PoolRegister register)
    {
        return PoolManager.InstanceAwake() == null ? null : PoolManager.Instance.RemovePoolExtension(register);
    }

    public static PoolManager RemovePoolKeyExtension(PoolEnums.PoolId poolId, bool IsDestory = true)
    {
        return PoolManager.InstanceAwake() == null ? null : PoolManager.Instance.RemovePoolKeys(poolId, IsDestory);
    }
}
